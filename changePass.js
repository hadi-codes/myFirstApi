        ...
        MongoClient.connect(url, { useNewUrlParser: true }).then((db) => {
            ...
            db.db('myuserdb').collection('user').findOne({ 'email': `${reqBody.email}` }).then((docs) => {
                ...
                if (docs != null) {
                    ...
                    argon2.verify(docs.password, reqBody.currentPass).then((isHashOk) => {
                        ...
                        checkPassPromise.then(() => {
                            argon2.hash(reqBody.newPass).then((hash) => {
                                console.log('6-upadating the pass')

                                db.db('myuserdb').collection('user').updateOne({ 'email': `${reqBody.email}` }, { $set: { password: hash } }).then(() => { callback({code:200,msg:'password changed successfully'});cloesDB=true ;db.close()})
                            ...
                            callback({msg:'current password is wrong'})
                            console.log(err)
                        })
                    ...
                    db.close()
                    callback({code:401,msg:'email error'})
